...
def analyze_f821_errors(f821_errors: List[Tuple[str, int, str]]) -> Dict[str, List[Tuple[str, int, str]]]:
    ...
    stdlib_modules = {
        'os', 'sys', 'pathlib', 'json', 'datetime', 'time', 'logging',
        'collections', 'itertools', 'functools', 'operator', 're',
        'urllib', 'http', 'socket', 'threading', 'asyncio', 'typing'
    }
    ...
    for file_path, line_num, message in f821_errors:
        ...
        if undefined_name in typing_imports:
            ...
            categories['missing_standard_library'].append((file_path, line_num, message))
...
def generate_analysis_report(errors_by_code: Dict[str, List], f821_categories: Dict[str, List]) -> str:
    ...
    report.append(f"- **F821 percentage of total**: {f821_count/total_errors*100:.1f}%")
    report.append("")
...
def main():
    ...
    if not Path(log_file).exists():
        print(f"Error: {log_file} not found. Please run 'ruff check . > ruff_errors.log' first.")
        return
    ...
    print(f"- Report saved to: LINTING_ANALYSIS.md")
