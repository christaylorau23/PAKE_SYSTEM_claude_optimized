...
try:
    ...
    print(
        "Warning: Could not import enterprise logging services. Using fallback examples."
    )
...
class LoggingExamples:
    ...
    def example_basic_logging(self):
        ...
        self.logger.debug(
            ...
            extra_data={"debug_info": "detailed debugging info"},
        )
    ...
    def example_structured_logging(self):
        ...
        with self.logger.with_correlation_id("req-12345"):
            ...
            with self.logger.with_user("user123", "john_doe"):
                self.logger.info(
                    "User action performed", action="login", ip="192.168.1.100"
                )
    ...
    def example_monitoring_metrics(self):
        ...
        self.monitor.increment_counter(
            "requests.success", tags={"endpoint": "/api/v1/users"}
        )
        self.monitor.increment_counter(
            "requests.error", tags={"error_type": "validation_error"}
        )
        ...
        self.monitor.record_timing(
            "api.response", 250.0, tags={"endpoint": "/api/v1/users"}
        )
    ...
    def example_alerting(self):
        ...
        # Resolve alert
        asyncio.create_task(
            self.monitor.resolve_alert(alert_id, "CPU usage returned to normal levels")
        )
    ...
    def example_configuration_management(self):
        ...
        print(
            f"Environment validation: {len(validation['validations'])} validations passed"
        )
    ...
    def example_error_handling_and_recovery(self):
        ...
        try:
            ...
            self.monitor.record_error(
                "unexpected_error", {"error_type": type(e).__name__}
            )
    ...
    def example_reporting(self):
        ...
        print(
            f"Compliance report: {compliance_report['summary']['total_audit_events']} audit events"
        )

        # Generate monitoring report
        monitoring_report = self.monitor.generate_monitoring_report()
        print(
            f"Monitoring report: {monitoring_report['system_metrics']['cpu']['current']}% CPU usage"
        )
