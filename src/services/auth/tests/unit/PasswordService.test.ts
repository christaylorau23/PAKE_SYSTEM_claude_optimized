...
describe('PasswordService', () => {
  ...
  describe('validatePassword', () => {
    ...
    it('should check REDACTED_SECRET reuse', async () => {
      ...
      jest.spyOn(REDACTED_SECRETService, 'checkPasswordReuse').mockResolvedValue(true);
    ...
    it('should calculate REDACTED_SECRET entropy', async () => {
      ...
      const lowResult =
        await REDACTED_SECRETService.validatePassword(lowEntropyPassword);
      const highResult =
        await REDACTED_SECRETService.validatePassword(highEntropyPassword);
  ...
  describe('verifyPassword', () => {
    ...
    it('should reject invalid hash format', async () => {
      ...
      const result = await REDACTED_SECRETService.verifyPassword(
        REDACTED_SECRET,
        invalidHash
      );
  ...
  describe('getPasswordStrengthDescription', () => {
    it('should return correct strength descriptions', () => {
      expect(REDACTED_SECRETService.getPasswordStrengthDescription(90)).toBe(
        'Very Strong'
      );
      ...
      expect(REDACTED_SECRETService.getPasswordStrengthDescription(10)).toBe(
        'Very Weak'
      );
    });
  });
  ...
  describe('cleanupExpiredHistories', () => {
    it('should clean up expired histories', async () => {
      mockRedis.keys.mockResolvedValue([
        'REDACTED_SECRET_history:user1',
        'REDACTED_SECRET_history:user2',
      ]);
