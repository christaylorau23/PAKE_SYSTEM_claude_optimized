...
class Phase1F821Fixer:
    ...
    def _parse_text_errors(
        self, output: str, errors_by_file: dict[str, list[tuple[int, str]]]
    ):
    ...
    def fix_file(
        ...
        try:
            ...
            undefined_names = {name for _, name in errors}

            # Fix missing imports
            if self._needs_import_fixes(undefined_names):
                lines = self._add_missing_imports(lines, undefined_names)
                fixes_applied += len(
                    [name for name in undefined_names if name in self.common_fixes]
                )
            ...
            for line_num, undefined_name in errors:
                if line_num <= len(lines):
                    ...
                    if self._is_parameter_error(line, undefined_name):
                        lines[line_num - 1] = self._fix_parameter_error(
                            line, undefined_name
                        )
                        fixes_applied += 1
    ...
    def _add_missing_imports(
        self, lines: list[str], undefined_names: set[str]
    ) -> list[str]:
