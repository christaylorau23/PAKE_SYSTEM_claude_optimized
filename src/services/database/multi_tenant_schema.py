...
class MultiTenantPostgreSQLService:
    ...
    def __init__(self, config: MultiTenantDatabaseConfig):
        ...
        self._async_url = f"postgresql+asyncpg://{config.username}:{config.password}@{config.host}:{config.port}/{config.database}"
        self._sync_url = f"postgresql://{config.username}:{config.password}@{config.host}:{config.port}/{config.database}"
