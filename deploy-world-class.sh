...
initialize_vault() {
    ...
    docker-compose exec -T vault /vault-init.sh || {
        ...
        docker-compose exec -T vault vault kv put secret/wealth-platform/postgres \
            database="wealth_db" \
            username="postgres" \
            password="${POSTGRES_PASSWORD:-$(openssl rand -base64 32)}" \
            replication-password="${POSTGRES_REPLICATION_PASSWORD:-$(openssl rand -base64 32)}"
        ...
        docker-compose exec -T vault vault kv put secret/wealth-platform/grafana \
            admin-password="${GRAFANA_ADMIN_PASSWORD:-$(openssl rand -base64 32)}"
    }
...
- **Redis:** localhost:6379 (password: WealthRedis!2025)
...
show_access_info() {
    ...
    echo -e "   Redis:           ${GREEN}localhost:6379${NC} (password: WealthRedis!2025)"
