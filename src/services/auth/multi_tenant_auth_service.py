...
class MultiTenantAuthService:
    ...
    - Account security (lockouts, password policies)
    ...
    async def authenticate_user(self, request: LoginRequest) -> LoginResponse:
        ...
        try:
            ...
            # Verify password
            try:
                self.password_hasher.verify(user["password_hash"], request.password)
            ...
            # Check if password needs rehashing (Argon2 upgrade)
            if self.password_hasher.check_needs_rehash(user["password_hash"]):
                new_hash = self.password_hasher.hash(request.password)
                await self._update_user_password_hash(tenant_id, user["id"], new_hash)
    ...
    async def change_password(
        ...
        try:
            ...
            try:
                self.password_hasher.verify(user["password_hash"], current_password)
            except VerifyMismatchError:
            ...
            # Update password
            await self._update_user_password_hash(tenant_id, user_id, new_password_hash)
    ...
    async def _update_user_password_hash(
        self,
        tenant_id: str,
        user_id: str,
        password_hash: str,
        ...
        await self.db_service.execute_query(
            "UPDATE users SET password_hash = $1, updated_at = $2 WHERE tenant_id = $3 AND id = $4",
            password_hash,
            datetime.utcnow(),
            tenant_id,
            user_id,
