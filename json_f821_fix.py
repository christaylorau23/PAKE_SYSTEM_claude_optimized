...
def fix_missing_init_parameters(
    file_path: Path, errors: list[dict]
) -> tuple[bool, int]:
    ...
    for init_line in reversed(init_lines):
        ...
        for error in errors:
            error_line = (
                error.get("location", {}).get("row", 0) - 1
            )  # Convert to 0-based
        ...
        if method_errors:
            ...
            lines[init_line] = lines[init_line].replace(
                ...
                f"def __init__(self, {param_str}) -> None:",
            )
            fixes_applied += 1
            print(
                f"  Fixed __init__ on line {init_line + 1}: added {len(set(method_errors))} parameters"
            )
...
def fix_missing_function_parameters(
    file_path: Path, errors: list[dict]
) -> tuple[bool, int]:
    ...
    for i, line in enumerate(lines):
        if (
            "@app." in line and "async def" in lines[i + 1]
            if i + 1 < len(lines)
            else False
        ):
            func_line = lines[i + 1]
            if "def " in func_line and "(self) -> None:" in func_line:
                ...
                for error in errors:
                    ...
                    if (
                        i + 1 <= error_line <= i + 20
                    ):  # Within 20 lines of function start
                        undefined_name = (
                            error.get("message", "").split("`")[1]
                            if "`" in error.get("message", "")
                            else ""
                ...
                if func_errors:
                    ...
                    lines[i + 1] = lines[i + 1].replace(
                        "(self) -> None:", f"(self, {param_str}) -> None:"
                    )
                    fixes_applied += 1
                    print(
                        f"  Fixed {func_name}: added {len(set(func_errors))} parameters"
                    )
