...
class EnvironmentSecretBackend(SecretBackend):
    ...
    def _validate_production_secrets(self):
        ...
        if len(db_password) < 12:
            logger.warning("DB_PASSWORD is shorter than 12 characters - consider using a stronger password")
        if len(redis_password) < 12:
            logger.warning("REDIS_PASSWORD is shorter than 12 characters - consider using a stronger password")
    ...
    async def get_secret(self, secret_name: str) -> Optional[str]:
        ...
        if value:
            ...
            if any(sensitive in secret_name.lower() for sensitive in ['password', 'key', 'secret', 'token']):
...
class SecretManager:
    ...
    async def get_service_credentials(self, service: str) -> Optional[Dict[str, Any]]:
        """Get service credentials (username/password pairs)"""
        if self.config.backend == "env":
            # Development: use environment variables
...
async def get_database_url() -> str:
    ...
    return f"postgresql://{config['user']}{password_part}@{config['host']}:{config['port']}/{config['database']}"
...
async def get_redis_url() -> str:
    ...
    password_part = f":{config['password']}@" if config['password'] else ""
    return f"redis://{password_part}{config['host']}:{config['port']}"