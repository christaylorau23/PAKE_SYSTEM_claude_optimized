...
check_hardcoded_secrets() {
    ...
    if find k8s/ -name "*.yaml" -not -path "*/multitenant/*" -not -path "*/overlays/*" -not -path "*/base/*" -exec grep -l "kind: Secret" {} \; | xargs grep -l "data:" | xargs grep -E "(password|key|token|secret)" | grep -v "# Secret is now managed" | grep -v "{{.*}}"; then
    ...
    if grep -r "WealthPass\|WealthDashboard\|ReplicaPass" k8s/ --exclude="*.md" --exclude="*.sh" --exclude="validate-security.sh"; then
        log_error "Found hardcoded passwords in manifests"
        issues_found=$((issues_found + 1))
    fi
