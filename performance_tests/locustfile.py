...
class WebAppUserBehavior(BaseUserBehavior):
    ...
    @task(6)
    def view_protected_content(self) -> None:
    ...
    @task(4)
    def view_admin_panel(self) -> None:
...
class ApiUserBehavior(BaseUserBehavior):
    ...
    @task(8)
    def protected_api_call(self) -> None:
    ...
    @task(4)
    def password_operations(self) -> None:
        ...
        self.client.post(
            "/auth/validate-password", json={"password": "TestPassword123!@#"}
        )

    @task(3)
    def error_scenarios(self) -> None:
        """Test error handling."""
        # Test 404
...
class ResearcherUserBehavior(BaseUserBehavior):
    ...
    @task(20)
    def authenticated_operations(self) -> None:
        ...
        if operation == "user_profile":
            ...
            self.client.post(
                "/auth/validate-password", json={"password": "SecureTestPassword123!@#"}
            )

    @task(8)
    def quick_operations(self) -> None:
    ...
    @task(5)
    def batch_profile_checks(self) -> None:
...
class AdminUserBehavior(BaseUserBehavior):
    ...
    @task(8)
    def system_monitoring(self) -> None:
    ...
    def password_management(self) -> None:
        ...
        # Validate password strength
        test_passwords = ["weak", "StrongerPass123", "VerySecure!Pass123@"]

        for password in test_passwords:
            self.client.post("/auth/validate-password", json={"password": password})
            time.sleep(0.1)
...
def validate_performance(results: dict) -> dict[str, bool]:
    ...
    validation["response_time"] = (
        avg_response_time <= PERFORMANCE_THRESHOLDS["max_response_time_ms"]
    )
    ...
    validation["error_rate"] = (
        error_rate <= PERFORMANCE_THRESHOLDS["max_error_rate_percent"]
    )

    # Check throughput (requests per second)
    throughput = total_requests / (
        results.get("test_duration", 1) / 60
    )  # Convert to RPS
    validation["throughput"] = (
        throughput >= PERFORMANCE_THRESHOLDS["min_throughput_rps"]
    )
