...
export class UserRoutes {
  ...
  private async changePassword(req: Request, res: Response): Promise<void> {
    try {
      ...
      this.logger.error('Change password error', { error: error.message });
  ...
  private async getPasswordRequirements(
    ...
    try {
      const requirements =
        this.services.passwordService.getPasswordRequirements();
      ...
      this.logger.error('Get password requirements error', {
        error: error.message,
      });
      res.status(500).json({ error: 'Internal server error' });
    }
  ...
  private async checkPasswordStrength(
    ...
    try {
      ...
      const validation = await this.services.passwordService.validatePassword(
        password,
        userId
      );
      ...
      res.json({
        ...
        strength: this.services.passwordService.getPasswordStrengthDescription(
          validation.score
        ),
      ...
      this.logger.error('Check password strength error', {
        error: error.message,
      });
      res.status(500).json({ error: 'Internal server error' });
    }
  ...
  private getPasswordSuggestions(errors: string[]): string[] {
    ...
    if (errors.some(e => e.includes('characters long'))) {
      suggestions.push('Use a longer password with at least 12 characters');
    }
    ...
    if (errors.some(e => e.includes('common password'))) {
