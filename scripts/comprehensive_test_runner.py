...
class PAKETestRunner:
    ...
    def check_prerequisites(self) -> bool:
        ...
        if missing_vars:
            self.log(
                f"Missing environment variables: {', '.join(missing_vars)}", "WARNING"
            )
    ...
    def run_security_tests(self) -> TestExecutionResult:
        ...
        try:
            cmd = ["poetry", "run", "python", self.test_config["security"]["path"]]
    ...
    def generate_report(self) -> dict:
        ...
        report = {
            ...
            if all(r.result == TestResult.PASSED for r in self.results)
            else "FAILED",
        }
    ...
    def print_summary(self):
        ...
        for mode, result in report["results_by_mode"].items():
            ...
            print(
                f"  {status_emoji} {mode.upper()}: {result['result'].upper()} "
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick tests only (unit tests)"
    )
