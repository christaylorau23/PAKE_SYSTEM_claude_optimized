...
def validate_password_strength(password: str, min_length: int = 12) -> dict[str, Any]:
...
def validate_production_environment() -> bool:
    ...
    required_vars = {
        ...
        "DB_PASSWORD": {
            "default": None,
            "validator": lambda x: validate_password_strength(x)["valid"],
        },
        ...
        "REDIS_PASSWORD": {
            "default": None,
            "validator": lambda x: validate_password_strength(x, 8)["valid"],
        },
    }
    ...
    for var_name, config in required_vars.items():
        ...
        if config["validator"] and not config["validator"](value):
            if "PASSWORD" in var_name:
                ...
                logger.error(
                    f"❌ {var_name}: Weak password - {', '.join(result['issues'])}",
                )
            else:
