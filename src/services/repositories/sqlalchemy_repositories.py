...
class BaseRepository(AbstractRepository[T], ABC):
    ...
    async def create(self, entity: T) -> T:
        ...
        try:
            async with self._session_maker() as session:
                ...
                logger.debug(
                    f"Created {self._model_class.__name__}: {domain_entity.id}"
                )
    ...
    async def get_by_id(self, entity_id: str) -> Optional[T]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get {self._model_class.__name__} by ID {entity_id}: {e}"
            )
            raise
    ...
    async def update(self, entity: T) -> T:
        ...
        try:
            async with self._session_maker() as session:
                ...
                logger.debug(
                    f"Updated {self._model_class.__name__}: {domain_entity.id}"
                )
                return domain_entity
        except Exception as e:
    ...
    async def delete(self, entity_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete {self._model_class.__name__} {entity_id}: {e}"
            )
            raise
    ...
    async def list(self, limit: int = 100, offset: int = 0) -> list[T]:
        ...
        try:
            async with self._session_maker() as session:
                ...
                logger.debug(
                    f"Listed {len(domain_entities)} {self._model_class.__name__} entities"
                )
