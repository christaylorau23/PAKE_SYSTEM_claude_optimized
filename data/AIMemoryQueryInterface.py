...
class MemoryQuery(BaseModel):
    ...
    context_id: str | None = Field(None, description="Conversation or context ID")
    memory_types: list[str] | None = Field(
        None, description="Types of memories to search"
    )
...
class AIMemoryQueryInterface:
    ...
    async def ask_memory(
        ...
        try:
            ...
            if cache_key in self.query_cache:
                ...
                if (datetime.now(UTC).timestamp() - timestamp) < self.cache_ttl:
    ...
    async def _enhance_query_results(
        ...
        results["suggestions"] = await self._generate_query_suggestions(
            results, original_query
        )

        # Add summary if we have good matches
        if results.get("direct_matches"):
            results["summary"] = await self._generate_result_summary(
    ...
    async def _generate_query_suggestions(
        ...
        if not all_matches:
            return [
                {
                    "query": f"What do you know about {original_query}?",
                    "type": "exploratory",
                },
                {
                    "query": f"Recent information about {original_query}",
                    "type": "temporal",
                },
        ...
        suggestions.extend(
            [
                ...
                {
                    "query": f"Historical context for {original_query}",
                    "type": "historical",
                },
            ]
        )
    ...
    async def learn_from_interaction(
        ...
        try:
            interaction_metadata = {
                ...
                "learning_weight": min(
                    1.0, abs(feedback_score)
                ),  # Higher weight for strong feedback
                **(metadata or {}),
            }
    ...
    async def get_conversation_history(
        ...
        try:
            context = await self.vector_db.get_conversation_context(
                conversation_id=conversation_id, context_window=context_window
            )
...
def create_memory_api(memory_interface: AIMemoryQueryInterface) -> FastAPI:
    ...
    async def get_conversation(
        conversation_id: str, context_window: int = Query(10, ge=1, le=50)
    ):
    ...
    async def cleanup_old_memories(max_age_days: int = Query(365, ge=1)):
        ...
        try:
            cleanup_stats = await memory_interface.vector_db.cleanup_old_memories(
                max_age_days
            )
            return JSONResponse(
                content={
...
async def create_memory_interface(
    persist_directory: str = None,
) -> AIMemoryQueryInterface:
