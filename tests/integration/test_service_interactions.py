...
class TestServiceIntegration:
    ...
    async def test_database_cache_integration(self, test_database, test_cache):
        ...
        cached_data_after_invalidation = await test_cache.get("users", user_data["user_id"])
    ...
    async def test_cache_invalidation_integration(self, test_cache, test_database):
        ...
        # Verify cache is invalidated
        cached_data_after_update = await test_cache.get("users", initial_data["user_id"])
    ...
    @pytest.mark.asyncio
    async def test_service_coordination_integration(self, test_message_bus, test_services):
        """
        ...
        # Execute workflow
        await test_message_bus.publish("workflow:step1", {
        ...
        await test_message_bus.publish("workflow:step2", {
            "workflow_id": "integration_test_workflow",
    ...
    @pytest.mark.asyncio
    async def test_authentication_database_integration(self, test_database, test_services):
        """
    ...
    async def test_complete_ingestion_workflow_integration(
        ...
        with patch.object(test_services["firecrawl"], 'extract_content') as mock_firecrawl, \
            ...
            mock_arxiv.return_value = {
                "success": True,
                "papers": [
                    {"title": "Integration Testing Paper 1", "abstract": "Testing integration patterns"},
                    {"title": "Integration Testing Paper 2", "abstract": "Advanced integration testing"}
                ],
                "total_results": 2
            }
...
class TestErrorPropagationIntegration:
    ...
    async def test_database_error_propagation(self, test_services):
        ...
        with patch('services.database.connection_manager.DatabaseConnectionManager.connect') as mock_connect:
    ...
    async def test_cache_error_propagation(self, test_database):
        ...
        with patch('services.caching.redis_cache_strategy.RedisCacheStrategy.get') as mock_cache_get:
