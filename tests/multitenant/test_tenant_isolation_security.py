...
class MultiTenantSecurityTester:
    ...
    async def _test_password_policies(self):
        ...
        for weak_password in weak_passwords:
            result = await self._test_weak_password_rejection(tenant_id, weak_password)
            self.test_results["authentication"].append(result)

    async def _test_weak_password_rejection(
        self,
        tenant_id: str,
        weak_password: str,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "password": weak_password,
                "status": "PASSED",
                "message": f"Weak password rejected: {str(e)}",
            }
    ...
    async def _test_account_lockouts(self):
        ...
        # Test that correct password is now rejected due to lockout
        correct_login = LoginRequest(
            username=admin_user["username"],
            password="correct_password",  # This would be the real password
            tenant_id=tenant_id,
        )
