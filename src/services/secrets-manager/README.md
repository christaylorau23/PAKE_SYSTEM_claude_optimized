...
console.log(secret.value); // { host: 'localhost', username: 'admin', password: 'secure_database_password' }
...
// Manual rotation
const result = await rotation.rotateNow('myapp/database', 'database-password');
```
...
const result = await breakGlass.executeEmergencyAction(
  sessionId,
  'secret.reveal',
  { secretPath: '/critical/database-password' }
);
```
