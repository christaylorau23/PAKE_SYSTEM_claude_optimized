...
1. **Registration Form**: Multi-step validation with password confirmation
...
function MyForm() {
  ...
  return (
    <Form formContextValue={formContextValue} onSubmit={handleSubmit}>
      ...
      <Input name="password" type="process.env.PAKE_WEAK_PASSWORD || 'SECURE_WEAK_PASSWORD_REQUIRED''SECURE_WEAK_PASSWORD_REQUIRED'" showPasswordToggle />
