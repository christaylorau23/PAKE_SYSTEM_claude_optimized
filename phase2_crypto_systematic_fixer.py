...
class Phase2CryptoFixer:
    ...
    def _fix_random_randint(self, content: str) -> str:
        ...
        return re.sub(
            r"secrets\.randbelow\(([^,]+),\s*([^)]+)\)", replace_randint, content
        )
    ...
    def _fix_random_uniform(self, content: str) -> str:
        ...
        return re.sub(
            r"secrets\.randbelow\(([^,]+),\s*([^)]+)\)", replace_uniform, content
        )
...
'''
    ...
    def run_systematic_crypto_fix(self) -> dict[str, Any]:
        ...
        print(
            f"📊 Found {total_errors} S311 weak crypto errors across {len(errors_by_file)} files"
        )
            ...
            if "*" in file_pattern:
                ...
                for file_path in self.project_root.glob(file_pattern):
                    if file_path.is_file() and str(file_path) in errors_by_file:
                        ...
                        success, fixes = self.fix_file_crypto(
                        ...
                        if success:
                            ...
                            print("   ❌ Failed to fix crypto issues")
            else:
