...
class UserService:
    ...
    async def create_user(
        ...
        try:
            ...
            auth_result = await self.auth_service.create_user(
                {"email": email, "password": password, "user_data": user_data}
            )
            ...
            return ServiceResult(
                ...
                metadata={
                    "notification_sent": notification_result.status
    ...
    async def get_user_profile(self, user_id: str) -> ServiceResult[dict[str, Any]]:
        ...
        try:
            ...
            if not user:
                return ServiceResult(
                    status=ServiceStatus.FAILED, error=f"User {user_id} not found"
                )
    ...
    def _validate_user_data(
        ...
        if "role" in user_data and user_data["role"] not in [
            role.value for role in UserRole
        ]:
            errors.append(f"Invalid role: {user_data['role']}")

        return ValidationResult(
            is_valid=len(errors) == 0, error="; ".join(errors) if errors else None
        )
