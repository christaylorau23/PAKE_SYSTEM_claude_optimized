...
class TestKubernetesDeploymentTDD:
    ...
    def test_helm_values_environment_specific(self):
        ...
        # Act
        with open(staging_values_path) as f:
        ...
        staging_debug = any(
            env.get("name") == "DEBUG" and env.get("value") == "true"
        ...
        production_debug = any(
            env.get("name") == "DEBUG" and env.get("value") == "false"
        ...
        assert production_resources.get("limits", {}).get(
            "cpu"
        ) > staging_resources.get("limits", {}).get(
            "cpu"
    ...
    def test_helm_deployment_template(self):
        ...
        assert (
            '{{ include "pake-system.fullname" . }}' in content
    ...
    def test_helm_service_template(self):
        ...
        assert (
            "type: {{ .Values.service.type }}" in content
        ), "Should use service type from values"
        assert (
    ...
    def test_helm_helpers_template(self):
        ...
        # Act
        with open(helpers_template_path) as f:
            content = f.read()
    ...
    def test_argocd_application_configuration(self):
        ...
        # Act
        with open(staging_app_path) as f:
            app_data = yaml.safe_load(f)

        # Assert
        assert app_data["kind"] == "Application", "Should be ArgoCD Application"
        assert (
            app_data["metadata"]["name"] == "pake-system-staging"
        ...
        source = spec["source"]
        assert (
            source["repoURL"] == "https://github.com/your-org/pake-system"
        ), "Should have correct repo URL"
    ...
    def test_kubernetes_security_configuration(self):
        ...
        assert (
            "securityContext" in values_data
