...
export class OllamaProvider implements AgentProvider {
  ...
  getStats() {
    return {
      ...
      avgTokensPerExecution:
        this.executionCount > 0 ? this.totalTokensGenerated / this.executionCount : 0,
  ...
  private async initializeModels(): Promise<void> {
    try {
      ...
      if (!this.availableModels.has(this.currentModel)) {
        ...
        if (!this.availableModels.has(this.currentModel)) {
          console.warn(
            `No configured models available. Available models: ${Array.from(this.availableModels).join(', ')}`
          );
        }
      }
    } catch (error) {
  ...
  private validateTask(task: AgentTask): void {
    ...
    if (estimatedTokens > this.config.numCtx * 0.8) {
    ...
    for (let attempt = 0; attempt < this.config.retries; attempt++) {
      try {
        ...
        if (attempt < this.config.retries - 1) {
          ...
          console.warn(
            `Ollama call failed (attempt ${attempt + 1}), retrying in ${delay}ms:`,
            error
          );
  ...
  private async makeOllamaCall(
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new AgentExecutionError('Ollama call timed out', AgentErrorCode.TIMEOUT, this.name);
      }
