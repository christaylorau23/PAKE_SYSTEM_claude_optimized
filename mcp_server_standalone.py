...
try:
    from services.analytics.advanced_analytics_engine import (
...
class SearchRequest(BaseModel):
    ...
    enable_ml_enhancement: bool = Field(
        default=True, description="Enable semantic search and ML enhancements"
    )
    enable_content_summarization: bool = Field(
        default=False, description="Enable advanced content summarization for results"
    )
...
class SummarizeRequest(BaseModel):
    ...
    content_type: str = Field(
        default="general",
        description="Type of content (academic, web, medical, general)",
    )
    target_sentences: int = Field(
        default=3, ge=1, le=10, description="Number of sentences for extractive summary"
    )
...
async def startup_event(self) -> None:
    ...
    try:
        ...
        if (
            firecrawl_api_key
            and firecrawl_api_key != "demo-key-for-testing-replace-with-real-key"
        ):
            logger.info("✅ Real Firecrawl API key detected")
        else:
            logger.info(
                "⚠️ Using demo mode - configure FIRECRAWL_API_KEY for real web scraping"
            )
    ...
    try:
        ...
        plan = await orchestrator.create_ingestion_plan(
            ...
            context={
                ...
                "max_results_per_source": search_request.max_results,
            },
        )
        ...
        if search_request.enable_ml_enhancement and results_data:
            try:
                ...
                (
                    enhanced_results,
                    ml_analytics,
                ) = await semantic_service.enhance_search_results(
        ...
        if search_request.enable_content_summarization and results_data:
            try:
                ...
                summarized_results = (
                    await summarization_service.summarize_research_results(results_data)
                )
                results_data = summarized_results
                summarization_analytics = (
                    summarization_service.get_summarization_analytics()
                )
                logger.info(
    ...
    try:
        ...
        if ml_analytics:
            summary["ml_analytics"] = {
                "avg_semantic_score": round(ml_analytics.avg_semantic_score, 3),
                "processing_time_ms": round(ml_analytics.processing_time_ms, 1),
                "top_topics": ml_analytics.top_topics[
                    :3
                ],  # Top 3 topics for quick search
            }
...
async def get_enhanced_dashboard(self) -> None:
    ...
    try:
        ...
        dashboard_data = await viz_service.get_enhanced_dashboard_data(
            time_range=time_range, metric_types=metric_list
        )
...
async def get_comprehensive_analytics_report(self) -> None:
    ...
    try:
        ...
        return await advanced_analytics_engine.generate_comprehensive_report(
            time_range=time_range,
...
async def get_system_health_analysis(self) -> None:
    ...
    try:
        ...
        health_score = await advanced_analytics_engine._analyze_system_health(
            time_range
        )
        return {
            "system_health": {
...
async def get_analytics_insights(self) -> None:
    ...
    try:
        ...
        if priority != "all":
            insights = [
                insight for insight in insights if insight.get("priority") == priority
            ]
...
async def get_usage_pattern_analysis(self) -> None:
    ...
    try:
        usage_patterns = await advanced_analytics_engine._analyze_usage_patterns(
            time_range
        )
        return {
...
async def get_graph_visualization(self) -> None:
    ...
    try:
        ...
        return await kg_service.get_knowledge_graph_visualization(
...
async def health_check(self) -> None:
    ...
    health_status = {
        ...
        "components": {
            ...
            != "demo-key-for-testing-replace-with-real-key"
            else "demo_mode",
            "arxiv_api": "available",
...
async def obsidian_sync(self) -> None:
    ...
    try:
        ...
        logger.info("Obsidian sync event: %s - %s", event_type, filepath)
...
async def extract_metadata(self) -> None:
    ...
    try:
        ...
        if include_topics:
            ...
            metadata["topics"] = [
                {"term": term, "frequency": freq} for term, freq in topics
            ]
