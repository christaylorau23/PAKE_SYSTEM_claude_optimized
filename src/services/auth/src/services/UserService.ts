...
export class UserService {
  ...
  async findUserById(userId: string): Promise<User | null> {
    try {
      ...
      const { passwordHash, passwordUpdatedAt, ...user } = JSON.parse(userData);
  ...
  async changePassword(
    ...
    try {
      ...
      const isReused = await this.passwordService.checkPasswordReuse(
        userId,
        newPassword
      );
      if (isReused) {
      ...
      await this.passwordService.addToPasswordHistory(
        userId,
        user.passwordHash
      );
  ...
  async resetPassword(token: string, newPassword: string): Promise<boolean> {
    try {
      ...
      const resetData = await this.redis.get(`password_reset:${tokenHash}`);
