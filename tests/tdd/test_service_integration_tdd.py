...
class TestServiceIntegrationTDD:
    ...
    def test_service_template_app_structure(self):
        ...
        required_files = ["main.py", "core/config.py"]
    ...
    def test_service_template_config_py(self):
        ...
        # Assert
        assert (
            "from pydantic import BaseSettings" in content
        ), "Should use Pydantic settings"
    ...
    def test_service_template_poetry_validation(self):
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), f"Poetry config should be valid: {result.stderr}"
