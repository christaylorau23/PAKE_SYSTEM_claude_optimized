...
kubectl patch secret database-connection -n pake-api --context=secondary --patch '{
  "data": {
    "DATABASE_URL": "'$(echo -n "postgresql://postgres:password@pake-postgresql-replica-eu.database.svc.cluster.local:5432/pake_production" | base64)'"
  }
}'
```
...
curl -X POST https://api.pake-system.com/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email": "test@pake-system.com", "password": "test123"}'
