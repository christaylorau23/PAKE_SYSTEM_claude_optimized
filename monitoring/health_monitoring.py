...
class HealthMetric(BaseModel):
    ...
    status: HealthStatus = Field(
        default=HealthStatus.HEALTHY, description="Health status"
    )
...
class MaintenanceTask(BaseModel):
    ...
    next_run: datetime | None = Field(None, description="Next run timestamp")
...
class SystemHealthReport(BaseModel):
    ...
    last_maintenance: datetime | None = Field(
        None, description="Last maintenance timestamp"
    )
...
class HealthMonitoringSystem:
    ...
    def _initialize_default_tasks(self) -> None:
        ...
        self.logger.info("Initialized %s default maintenance tasks", len(default_tasks))

    def _initialize_service_checks(self) -> None:
    ...
    async def collect_system_metrics(self) -> list[HealthMetric]:
        ...
        metrics.append(
            HealthMetric(
                ...
                status=self._get_metric_status(
                    memory.available / (1024 * 1024), 1000.0, 500.0, reverse=True
                ),
            )
        )
        ...
        metrics.append(
            HealthMetric(
                ...
                threshold_critical=95.0,
                status=self._get_metric_status(
                    (disk.used / disk.total) * 100, 80.0, 95.0
                ),
            )
        )
        ...
        metrics.append(
            HealthMetric(
                ...
                status=self._get_metric_status(
                    disk.free / (1024 * 1024 * 1024), 10.0, 5.0, reverse=True
                ),
            )
        )
        ...
        self.logger.info("Collected %s system metrics", len(metrics))
        return metrics
    ...
    async def check_service_health(self, service_name: str) -> HealthStatus:
        ...
        if service_name not in self.service_checks:
            self.logger.warning("No health check defined for service: %s", service_name)
    ...
    async def generate_health_report(self) -> SystemHealthReport:
        ...
        recommendations = self._generate_recommendations(
            current_metrics, service_status
        )
        ...
        self.logger.info(
            "Generated health report: %s - %s", report.report_id, overall_status.value
        )
        return report

    def _determine_overall_status(
        self, metrics: list[HealthMetric], service_status: dict[str, HealthStatus]
    ) -> HealthStatus:
        """Determine overall system health status."""
        # Check for critical status
        critical_metrics = [m for m in metrics if m.status == HealthStatus.CRITICAL]
        critical_services = [
            s for s in service_status.values() if s == HealthStatus.CRITICAL
        ]
        ...
        degraded_services = [
            s for s in service_status.values() if s == HealthStatus.DEGRADED
        ]
        ...
        unhealthy_services = [
            s for s in service_status.values() if s == HealthStatus.UNHEALTHY
        ]
    ...
    def _generate_recommendations(
        ...
        # Check CPU usage
        cpu_metric = next(
            (m for m in metrics if m.metric_name == "cpu_usage_percent"), None
        )
        ...
        # Check memory usage
        memory_metric = next(
            (m for m in metrics if m.metric_name == "memory_usage_percent"), None
        )
        ...
        # Check disk usage
        disk_metric = next(
            (m for m in metrics if m.metric_name == "disk_usage_percent"), None
        )
        if disk_metric and disk_metric.value > 80:
            recommendations.append(
                "Consider cleaning up disk space or expanding storage capacity"
            )
        ...
        if failed_services:
            recommendations.append(
                f"Investigate and restart failed services: {', '.join(failed_services)}"
            )
    ...
    async def run_maintenance_task(self, task_id: str) -> bool:
        ...
        if not task:
            self.logger.error("Maintenance task not found: %s", task_id)
            return False

        if not task.enabled:
            self.logger.info("Maintenance task %s is disabled", task_id)
            return False

        self.logger.info("Starting maintenance task: %s", task.name)
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    "Maintenance task completed successfully: %s", task.name
                )
            else:
                task.failure_count += 1
                self.logger.error("Maintenance task failed: %s", task.name)
            ...
            self.logger.error("Maintenance task error: %s - %s", task.name, str(e))
            return False

    async def _get_task_by_id(self, task_id: str) -> MaintenanceTask | None:
    ...
    async def _execute_task(self, task: MaintenanceTask) -> bool:
        ...
        try:
            ...
            if task.task_type == MaintenanceTaskType.BACKUP:
                return await self._run_backup_task(task)
            if task.task_type == MaintenanceTaskType.CACHE_CLEAR:
                return await self._run_cache_clear_task(task)
            if task.task_type == MaintenanceTaskType.SECURITY_SCAN:
                return await self._run_security_scan_task(task)
            if task.task_type == MaintenanceTaskType.CAPACITY_CHECK:
                return await self._run_capacity_check_task(task)
            if task.task_type == MaintenanceTaskType.OPTIMIZATION:
            ...
            if log_dir.exists():
                ...
                for log_file in log_dir.rglob("*.log"):
                    if log_file.stat().st_mtime < cutoff_time.timestamp():
                        log_file.unlink()
                        self.logger.info("Deleted old log file: %s", log_file)
            ...
            if temp_dir.exists():
                ...
                for temp_file in temp_dir.glob("pake_*"):
                    if temp_file.stat().st_mtime < cutoff_time.timestamp():
                        temp_file.unlink()
                        self.logger.info("Deleted temporary file: %s", temp_file)
    ...
    async def _run_backup_task(self, task: MaintenanceTask) -> bool:
        ...
        try:
            ...
            backup_data = {
                "timestamp": timestamp,
                "health_metrics": [
                    metric.dict() for metric in self.health_metrics[-100:]
                ],
        ...
        try:
            ...
            if cache_dir.exists():
                for cache_file in cache_dir.glob("*"):
                    if cache_file.is_file():
                        cache_file.unlink()
                        self.logger.info("Cleared cache file: %s", cache_file)
            ...
            if temp_cache_dir.exists():
                for cache_file in temp_cache_dir.glob("*"):
                    if cache_file.is_file():
                        cache_file.unlink()
                        self.logger.info("Cleared temp cache file: %s", cache_file)
    ...
    async def _run_capacity_check_task(self, task: MaintenanceTask) -> bool:
        ...
        try:
            ...
            if disk_usage_percent > 90:
                self.logger.warning(
                    "Disk usage is high: %s", f"{disk_usage_percent:.1f}%"
                )

            # Check memory usage
            memory = psutil.virtual_memory()
            if memory.percent > 90:
                self.logger.warning("Memory usage is high: %.1f%%", memory.percent)
    ...
    async def run_scheduled_maintenance(self) -> None:
    ...
    def _should_run_task(self, task: MaintenanceTask, current_time: datetime) -> bool:
        ...
        if (
            "daily" in task.schedule
            and time_since_last_run.total_seconds() > 86400
            or "weekly" in task.schedule
            and time_since_last_run.total_seconds() > 604800
            or "hourly" in task.schedule
            and time_since_last_run.total_seconds() > 3600
        ):  # 24 hours
            return True
    ...
    async def start_monitoring(self) -> None:
        ...
        self.logger.info(
            "Starting health monitoring with %ss interval", interval_seconds
        )
        ...
        while self.monitoring_active:
            try:
                ...
                if report.overall_status != HealthStatus.HEALTHY:
                    self.logger.warning(
                        "System health: %s", report.overall_status.value
                    )
    ...
    async def get_health_trends(self, days: int = 7) -> dict[str, Any]:
        ...
        avg_uptime = sum(report.uptime_seconds for report in recent_reports) / len(
            recent_reports
        )
