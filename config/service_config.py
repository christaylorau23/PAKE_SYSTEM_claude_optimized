...
class ServiceConfig:
    ...
    def __init__(
        self, environment_prefix: str = "PAKE", config_file: str | None = None
    ) -> None:
        ...
        logger.info("ServiceConfig initialized with file: %s", self.config_file_path)

    def _load_configuration(self, config_file: str | None) -> None:
    ...
    def _find_config_file(self, config_file: str | Path | None) -> Path | None:
    ...
    def _load_config_file(
        self, config_file: str | Path | None
    ) -> dict[str, Any] | None:
    ...
    def _apply_config_overrides(self, config_data: dict) -> None:
            ...
            if section_name in config_data:
                ...
                for key, value in section_config.items():
                    if hasattr(section_obj, key):
                        ...
                        if (
                            isinstance(current_value, type(value))
                            or current_value is None
                        ):
                            setattr(section_obj, key, value)
                            logger.debug(
                                "Config override: %s.%s = %s", section_name, key, value
                            )
                        else:
                            logger.warning(
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        env_mappings = {
            ...
            f"{self.environment_prefix}MAX_FILENAME_LENGTH": (
                "vault",
                "max_filename_length",
            ),
            f"{self.environment_prefix}SUMMARY_LENGTH": (
                "vault",
                "summary_truncate_length",
            ),
            f"{self.environment_prefix}DEFAULT_CONFIDENCE": (
                "vault",
                "default_confidence_score",
            ),
            # Search configuration
            f"{self.environment_prefix}SEARCH_LIMIT": (
                "search",
                "default_search_limit",
            ),
            f"{self.environment_prefix}MAX_SEARCH_LIMIT": (
                "search",
                "max_search_limit",
            ),
            # Cache configuration
            f"{self.environment_prefix}CACHE_TTL": ("cache", "default_ttl_seconds"),
            f"{self.environment_prefix}CACHE_TTL_PROD": (
                "cache",
                "production_ttl_seconds",
            ),
            f"{self.environment_prefix}CACHE_TTL_DEV": (
                "cache",
                "development_ttl_seconds",
            ),
            # Logging configuration
            f"{self.environment_prefix}LOG_LEVEL": ("logging", "default_level"),
            f"{self.environment_prefix}LOG_JSON": ("logging", "json_formatting"),
            f"{self.environment_prefix}LOG_STACK_LINES": (
                "logging",
        ...
        for env_var, (section_name, attr_name) in env_mappings.items():
            ...
            if env_value is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "Invalid environment value for %s: %s (%s)",
                        env_var,
                        env_value,
                        e,
                    )
    ...
    def get_folder_for_note_type(self, note_type: str) -> str:
        ...
        return self.vault.folder_structure.get(
            note_type, self.vault.folder_structure["SourceNote"]
        )
    ...
    def validate_configuration(self) -> bool:
        ...
        if self.search.default_search_limit > self.search.max_search_limit:
            issues.append(
                "default_search_limit cannot be greater than max_search_limit"
            )
        ...
        if issues:
            for issue in issues:
                logger.error("Configuration validation error: %s", issue)
            return False
...
_config_instance: ServiceConfig | None = None
