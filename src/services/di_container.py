...
class DIContainer:
    ...
    def register_factory(self, interface: Type[T], factory: Callable[[], T], lifetime: ServiceLifetime = ServiceLifetime.TRANSIENT) -> 'DIContainer':
        ...
        logger.info(f"Registered factory: {interface_name} (lifetime: {lifetime.value})")
        return self
    ...
    def _register_service(self, interface: Type[T], implementation: Type[T], lifetime: ServiceLifetime) -> 'DIContainer':
        ...
        logger.info(f"Registered service: {interface_name} -> {implementation.__name__} (lifetime: {lifetime.value})")
        return self
    ...
    def _create_instance(self, registration: ServiceRegistration) -> T:
        ...
        if registration.implementation:
            try:
                ...
                logger.warning(f"Failed to resolve dependencies for {registration.implementation.__name__}: {e}")
                # Fallback to parameterless constructor
                return registration.implementation()
        ...
        for param_name, param in signature.parameters.items():
            ...
            if param.annotation != inspect.Parameter.empty:
                try:
                    ...
                    logger.warning(f"Cannot resolve dependency {param_name} of type {param.annotation}")
    ...
    def get_registered_services(self) -> Dict[str, str]:
        ...
        for name, registration in self._services.items():
            services[name] = f"{registration.implementation.__name__ if registration.implementation else 'Factory'} ({registration.lifetime.value})"
        
        # Add singleton instances
        for name in self._singletons.keys():
            services[name] = f"Instance (singleton)"
...
class MockNotificationService:
    ...
    async def send_notification(self, user_id: str, message: str, notification_type: str):
        from ..domain.interfaces import ServiceResult, ServiceStatus
        return ServiceResult(status=ServiceStatus.SUCCESS, data=True)
