...
class TestRunner {
  ...
  async run(options = {}) {
    console.log('🧪 PAKE System Test Runner');
  ...
  async runSuite(suiteName, options = {}) {
    ...
    try {
      ...
      // Parse results
      const suiteResult = await this.parseTestResults(
        suiteName,
        result,
        duration
      );
      ...
      if (suiteResult.success) {
        ...
        console.log(
          `   Tests: ${suiteResult.tests.passed}/${suiteResult.tests.total} passed`
        );
  ...
  buildJestCommand(suiteName, suite, options) {
    ...
    if (!options.noCoverage) {
      args.push('--coverage');
      args.push(
        `--coverageDirectory=${config.coverage.outputDir}/${suiteName}`
      );
    }
    ...
    if (options.json) {
      args.push('--json');
      args.push(
        `--outputFile=${config.reporting.outputDir}/${suiteName}-results.json`
      );
    }
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          if (error && error.code !== 1) {
            // Jest exits with 1 when tests fail
            reject(error);
          } else {
            resolve({
              stdout,
  ...
  async parseTestResults(suiteName, result, duration) {
    ...
    try {
      ...
      console.warn(
        `⚠️  Could not parse results for ${suiteName}:`,
        parseError.message
      );
    ...
    for (const line of lines) {
      ...
      if (line.includes('Tests:')) {
        const matches = line.match(
          /(\d+)\s+failed,?\s*(\d+)\s+passed,?\s*(\d+)\s+total/
        );
        if (matches) {
  ...
  async parseCoverageResults(suiteName) {
    ...
    try {
      ...
      console.warn(
        `⚠️  Could not parse coverage for ${suiteName}:`,
        error.message
      );
    }
  ...
  calculateOverallCoverage() {
    ...
    const totalCoverage = suites.reduce(
      (sum, suite) => sum + (suite.coverage?.overall || 0),
      0
    );
  ...
  printSummary() {
    ...
    for (const [suiteName, result] of Object.entries(this.results.suites)) {
      ...
      console.log(
        `  ${status} ${suiteName}: ${result.tests.passed}/${result.tests.total} tests, ${coverage}% coverage`
      );
    }
    ...
    const hasFailures =
      summary.failed > 0 ||
      Object.values(this.results.suites).some(s => !s.success);
    if (hasFailures) {
      console.log('\n❌ Some tests failed');
      process.exit(1);
    } else {
