...
validate_consistency() {
    ...
    # Check for hardcoded passwords in non-development configs
    if grep -r "changeme\|password123\|secret123" "$DEPLOY_DIR/k8s/overlays/production" "$DEPLOY_DIR/k8s/overlays/staging" 2>/dev/null; then
        error "Found hardcoded passwords in production/staging configurations"
        ((validation_errors++))
    else
        success "No hardcoded passwords found in production/staging"
    fi
