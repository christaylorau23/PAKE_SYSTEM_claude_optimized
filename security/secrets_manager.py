...
class SecretMetadata(BaseModel):
    ...
    rotation_schedule: str | None = Field(None, description="Rotation schedule")
...
class SecretsManager:
    ...
    def _validate_provider_parameter(self) -> None:
        ...
        if not isinstance(self.provider, SecretProvider):
            msg = f"Provider must be a SecretProvider enum value, got {type(self.provider)}"
            raise ValueError(msg)

        # Validate provider-specific requirements
        if self.provider == SecretProvider.AZURE_KEY_VAULT:
            if not os.getenv("AZURE_KEY_VAULT_URL"):
                msg = "AZURE_KEY_VAULT_URL environment variable is required for Azure Key Vault provider"
                raise ValueError(msg)
        elif self.provider == SecretProvider.GOOGLE_SECRET_MANAGER:
            if not os.getenv("GOOGLE_CLOUD_PROJECT_ID"):
                msg = "GOOGLE_CLOUD_PROJECT_ID environment variable is required for Google Secret Manager provider"
                raise ValueError(msg)
    ...
    def _configure_local_provider(self) -> None:
        ...
        self.provider_config = {
            ...
            "backup_enabled": os.getenv("LOCAL_SECRETS_BACKUP", "true").lower()
            == "true",
        }
    ...
    def _initialize_provider_client(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "✅ %s client initialized successfully", self.provider.value
            )

        except (ImportError, ModuleNotFoundError) as e:
            self.logger.error(
                "Missing required dependencies for %s: %s", self.provider.value, str(e)
            )
            msg = f"Required dependencies not installed for {self.provider.value}"
            raise ImportError(msg) from e
        except (ValueError, KeyError) as e:
            self.logger.error(
                "Invalid configuration for %s: %s", self.provider.value, str(e)
            )
            msg = f"Invalid provider configuration: {str(e)}"
            raise ValueError(msg) from e
        except (ConnectionError, TimeoutError) as e:
            self.logger.error(
                "Network error initializing %s: %s", self.provider.value, str(e)
            )
            msg = f"Failed to connect to {self.provider.value} service"
            raise ConnectionError(msg) from e
        except PermissionError as e:
            self.logger.error(
                "Permission denied for %s: %s", self.provider.value, str(e)
            )
            ...
            msg = f"Provider initialization failed: {str(e)}"
    ...
    def _initialize_aws_client(self) -> None:
        ...
        try:
            ...
            self.logger.error("Missing boto3 dependency: %s", str(e))
            msg = "boto3 is required for AWS Secrets Manager integration"
            raise ImportError(msg) from e
        except (ValueError, KeyError) as e:
            self.logger.error("Invalid AWS configuration: %s", str(e))
            msg = f"Invalid AWS configuration: {str(e)}"
            raise ValueError(msg) from e
        except (ConnectionError, TimeoutError) as e:
            self.logger.error("AWS connection error: %s", str(e))
            msg = f"Failed to connect to AWS Secrets Manager: {str(e)}"
            raise ConnectionError(msg) from e
        except PermissionError as e:
    ...
    def _initialize_azure_client(self) -> None:
        ...
        try:
            ...
            self.logger.error("Missing Azure SDK dependency: %s", str(e))
            msg = "azure-identity and azure-keyvault-secrets are required for Azure Key Vault integration"
            raise ImportError(msg) from e
        except (ValueError, KeyError) as e:
            self.logger.error("Invalid Azure configuration: %s", str(e))
            msg = f"Invalid Azure Key Vault configuration: {str(e)}"
            raise ValueError(msg) from e
        except (ConnectionError, TimeoutError) as e:
            self.logger.error("Azure connection error: %s", str(e))
            msg = f"Failed to connect to Azure Key Vault: {str(e)}"
            raise ConnectionError(msg) from e
        except PermissionError as e:
            ...
            msg = f"Azure client initialization failed: {str(e)}"
            raise RuntimeError(msg) from e
    ...
    def _initialize_google_client(self) -> None:
        ...
        try:
            ...
            self.logger.error("Missing Google Cloud SDK dependency: %s", str(e))
            msg = "google-cloud-secret-manager is required for Google Secret Manager integration"
            raise ImportError(msg) from e
        except (ValueError, KeyError) as e:
            self.logger.error("Invalid Google Cloud configuration: %s", str(e))
            msg = f"Invalid Google Secret Manager configuration: {str(e)}"
            raise ValueError(msg) from e
        except (ConnectionError, TimeoutError) as e:
            self.logger.error("Google Cloud connection error: %s", str(e))
            msg = f"Failed to connect to Google Secret Manager: {str(e)}"
            raise ConnectionError(msg) from e
        except PermissionError as e:
            ...
            msg = f"Google client initialization failed: {str(e)}"
            raise RuntimeError(msg) from e
    ...
    def _initialize_local_client(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                "File system error creating secrets directory: %s", str(e)
            )
            msg = f"Cannot create secrets directory: {str(e)}"
            raise PermissionError(msg) from e
        except (ValueError, KeyError) as e:
            self.logger.error("Invalid local storage configuration: %s", str(e))
            msg = f"Invalid local storage configuration: {str(e)}"
            raise ValueError(msg) from e
        except json.JSONDecodeError as e:
            self.logger.error("Metadata file JSON error: %s", str(e))
            msg = f"Cannot parse metadata file JSON: {str(e)}"
            raise ValueError(msg) from e
            ...
            msg = f"Local client initialization failed: {str(e)}"
            raise RuntimeError(msg) from e
        ...
        try:
            ...
            if not encrypted_value:
                await self._log_access(
                    ...
                    user_agent,
                    success=False,
                    error_message="Failed to retrieve encrypted secret",
                )
                return None
            ...
            await self._log_access(
                secret_id, "read", accessed_by, source_ip, user_agent, success=True
            )
    ...
    async def delete_secret(self, secret_id: str, deleted_by: str = "system") -> bool:
        """Delete a secret.
    ...
    async def _store_aws_secret(self) -> None:
        ...
        try:
            ...
            self.aws_client.update_secret(
                SecretId=secret_id, SecretString=encrypted_value
            )

    async def _retrieve_aws_secret(self, secret_id: str) -> str | None:
    ...
    async def _retrieve_azure_secret(self, secret_id: str) -> str | None:
    ...
    async def _retrieve_google_secret(self, secret_id: str) -> str | None:
    ...
    async def _retrieve_local_secret(self, secret_id: str) -> str | None:
        """Retrieve secret from local file."""
        secret_file = self.secrets_dir / f"{secret_id}.secret"
        if secret_file.exists():
            async with aiofiles.open(secret_file) as f:
                return await f.read()
        return None
    ...
    async def _log_access(self) -> None:
        ...
        if success:
            self.logger.info(
                "Secret access: %s %s by %s", access_type, secret_id, accessed_by
            )
        else:
            self.logger.warning(
                "Failed secret access: %s %s by %s - %s",
                access_type,
                secret_id,
                accessed_by,
                error_message,
    ...
    async def get_secret_metadata(self, secret_id: str) -> SecretMetadata | None:
        """Get metadata for a specific secret."""
        return self.metadata_store.get(secret_id)

    async def rotate_secret(
        self, secret_id: str, new_value: str, rotated_by: str = "system"
    ) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("Secret rotated successfully: %s", secret_id)
    ...
    async def get_access_logs(
        self, secret_id: str | None = None, days: int = 30
    ...
    async def generate_security_report(self, days: int = 30) -> dict[str, Any]:
        ...
        recent_logs = [
            log for log in self.access_logs if start_date <= log.accessed_at <= end_date
        ]
        ...
        most_accessed = sorted(access_counts.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        ...
        return {
            ...
            "summary": {
                ...
                "success_rate": (total_accesses - failed_accesses) / total_accesses
                if total_accesses > 0
                else 1.0,
            ...
            "recommendations": await self._generate_recommendations(
                recent_logs, expiring_secrets
            ),
        }

    async def _generate_recommendations(
        self, recent_logs: list[SecretAccessLog], expiring_secrets: list[SecretMetadata]
    ) -> list[str]:
        ...
        if len(failed_logs) > 10:
            recommendations.append(
                "High number of failed secret accesses detected - review access patterns"
            )

        # Check for expiring secrets
        if expiring_secrets:
            recommendations.append(
                f"{len(expiring_secrets)} secrets are expiring soon - schedule rotation"
            )

        # Check for secrets that haven't been accessed recently
        all_secret_ids = set(self.metadata_store.keys())
        accessed_secret_ids = {log.secret_id for log in recent_logs}
        unused_secrets = all_secret_ids - accessed_secret_ids

        if unused_secrets:
            recommendations.append(
                f"{len(unused_secrets)} secrets haven't been accessed recently - consider cleanup"
            )
