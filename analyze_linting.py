...
def parse_ruff_log(log_file: str) -> Dict[str, any]:
    ...
    with open(log_file, 'r') as f:
        for line in f:
            ...
            if clean_line.startswith('warning:') or clean_line.startswith('help:'):
                continue
            ...
            match = re.match(r'^([^:]+):(\d+):(\d+):\s+([A-Z0-9]+)(?:\s+\[[^\]]+\])?\s+(.+)$', clean_line)
            if match:
...
def generate_analysis_report(data: Dict[str, any]) -> str:
    ...
    report.append(f"- **Target**: {data['error_counts'].get('F821', 0):,} F821 undefined-name errors")
...
def main():
    ...
    if not Path(log_file).exists():
        print(f"Error: {log_file} not found. Run 'ruff check . > ruff_errors.log' first.")
        return
