...
class SystematicF821Resolver:
        ...
        if modified:
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                print(
                    f"✅ Fixed {len(needed_imports)} imports and {len(parameter_fixes)} parameters in {file_path}"
                )
    ...
    def _add_imports(self, content: str, needed_imports: set[str]) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith(('"""', "'''")):
                in_docstring = bool(not in_docstring)
                continue
    ...
    def _fix_parameter_issues(
        self, content: str, parameter_fixes: list[tuple[int, str]]
    ) -> str:
...
def main():
    ...
    print("📄 Report written to: F821_SYSTEMATIC_RESOLUTION_REPORT.md")
