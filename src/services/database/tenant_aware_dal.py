...
class TenantAwareRepository[T: Base](ABC):
    ...
    async def _validate_tenant_access(self, tenant_id: str) -> None:
        ...
        if current_tenant_id != tenant_id:
            msg = f"Access denied to tenant {tenant_id}"
            raise TenantIsolationError(msg)
    ...
    async def create(self, **kwargs) -> T:
        ...
        async with self._session_maker() as session:
            ...
            logger.debug("Created %s: %s", self.model_class.__name__, instance.id)
            return instance
    ...
    async def update(self, record_id: str, **kwargs) -> T | None:
        ...
        async with self._session_maker() as session:
            ...
            logger.debug("Updated %s: %s", self.model_class.__name__, instance.id)
            return instance
    ...
    async def delete(self, record_id: str) -> bool:
        ...
        async with self._session_maker() as session:
            ...
            logger.debug("Deleted %s: %s", self.model_class.__name__, instance.id)
            return True
...
class UserRepository(TenantAwareRepository[User]):
    ...
    async def update_last_login(self, user_id: str) -> bool:
        ...
        result = await self.update(user_id, last_login=datetime.now(UTC))
...
class TenantActivityRepository(TenantAwareRepository[TenantActivity]):
    ...
    async def log_activity(
        ...
        logger.debug("Logged activity: %s for tenant %s", activity_type, tenant_id)
        return activity
...
class TenantResourceUsageRepository(TenantAwareRepository[TenantResourceUsage]):
    ...
    async def record_usage(
        ...
        logger.debug("Recorded usage: %s for tenant %s", resource_type, tenant_id)
        return usage
