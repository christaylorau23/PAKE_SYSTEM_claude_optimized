...
from tests.factories import UserFactory, UserInDBFactory, LoginRequestFactory
...
class TestAuthenticationServiceComprehensive:
    ...
    async def test_user_login_success(self, auth_service, mock_services):
        ...
        # Act
        result = await auth_service.loginUser(login_data['username'], login_data['password'])
    ...
    async def test_token_refresh_success(self, auth_service, mock_services):
        ...
        mock_services['tokenService'].refreshToken.assert_called_once_with(refresh_token)
    ...
    async def test_user_logout_success(self, auth_service, mock_services):
        ...
        mock_services['sessionService'].destroySession.assert_called_once_with(session_id)
    ...
    async def test_password_reset_request_success(self, auth_service, mock_services):
        ...
        mock_services['passwordService'].generateResetToken.return_value = 'reset_token_123'
    ...
    async def test_password_reset_confirm_success(self, auth_service, mock_services):
        ...
        # Assert
        assert result is True
        mock_services['passwordService'].validateResetToken.assert_called_once_with(reset_token)
        mock_services['passwordService'].resetPassword.assert_called_once_with(reset_token, new_password)
    ...
    @pytest.mark.unit_edge_case
    async def test_login_with_case_insensitive_username(self, auth_service, mock_services):
    ...
    async def test_token_refresh_with_expired_token(self, auth_service, mock_services):
        ...
        mock_services['tokenService'].refreshToken.side_effect = jwt.ExpiredSignatureError("Token expired")
    ...
    @pytest.mark.unit_edge_case
    async def test_password_reset_with_nonexistent_email(self, auth_service, mock_services):
    ...
    async def test_registration_with_invalid_email(self, auth_service, mock_services):
        ...
        mock_services['userService'].createUser.side_effect = ValueError("Invalid email format")
    ...
    async def test_registration_with_weak_password(self, auth_service, mock_services):
        ...
        mock_services['userService'].createUser.side_effect = ValueError("Password does not meet security requirements")
    ...
    async def test_redis_connection_failure(self, auth_service, mock_services):
        ...
        # Arrange
        mock_services['redis'].connect.side_effect = Exception("Redis connection failed")
    ...
    async def test_email_service_failure(self, auth_service, mock_services):
        ...
        mock_services['userService'].getUserByEmail.return_value = user
        mock_services['passwordService'].generateResetToken.return_value = 'reset_token_123'
        mock_services['emailService'].sendPasswordResetEmail.side_effect = Exception("Email service unavailable")
