    ...
    for py_file in Path('.').rglob('*.py'):
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if re.search(r'\brandom\.(random|uniform|randint|choice|shuffle)\b', line):
                    violations.append((str(py_file), line_num, line.strip()))
                elif re.search(r'\bfrom random import', line):
                    violations.append((str(py_file), line_num, line.strip()))
                elif re.search(r'\bimport random\b', line):
                    violations.append((str(py_file), line_num, line.strip()))
...
def fix_random_imports(file_path: str) -> bool:
    ...
    try:
        ...
        replacements = [
            ...
            (r'\brandom\.uniform\(([^)]+)\)', r'secrets.randbelow(int((\1[1] - \1[0]) * 1000)) / 1000 + \1[0]'),
            (r'\brandom\.randint\(([^,]+),\s*([^)]+)\)', r'secrets.randbelow(\2 - \1 + 1) + \1'),
...
def main():
    ...
    for file_path, line_num, line_content in violations:
        if file_path not in fixed_files:
            if fix_random_imports(file_path):
