...
sudo -u postgres psql -c "ALTER USER pake_user PASSWORD 'secure_password';"
...
services:
  pake-api:
    ...
    ports:
      - "8000:8000"
    environment:
      - DATABASE_URL=postgresql://pake_user:password@postgres:5432/pake_system
...
data:
  DATABASE_URL: "postgresql://pake_user:password@postgres-service:5432/pake_system"
...
DATABASE_URL="postgresql://pake_user:password@localhost:5432/pake_system"
...
CREATE USER pake_readonly WITH PASSWORD 'readonly_password';
...
python -c "
import psycopg2
conn = psycopg2.connect('postgresql://pake_user:password@localhost:5432/pake_system')
conn.close()
print('Database OK')
" || exit 1
