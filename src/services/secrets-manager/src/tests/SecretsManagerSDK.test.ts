...
describe('SecretsManagerSDK', () => {
  ...
  describe('Secret Operations', () => {
    ...
    it('should retrieve secret and cache it', async () => {
      const mockSecret: Secret = {
        ...
        value: { username: 'admin', password: process.env.UNKNOWN },
        encrypted: false,
      };
    ...
    it('should retry failed operations', async () => {
      mockVaultService.getSecret
        ...
        .mockResolvedValueOnce({
          ...
          value: { username: process.env.UNKNOWN },
          encrypted: false,
        });
    ...
    it('should fail after max retry attempts', async () => {
      mockVaultService.getSecret.mockRejectedValue(
        new Error('Persistent error')
      );

      const request: SecretRequest = { path: 'myapp/database' };

      await expect(sdk.getSecret(request)).rejects.toThrow(
  ...
  describe('Secret Injection', () => {
    ...
    it('should throw for missing required secrets', async () => {
      ...
      await expect(sdk.injectSecrets(secrets)).rejects.toThrow(
  ...
  describe('Error Handling', () => {
    ...
    it('should handle vault service unavailable', async () => {
      mockVaultService.getHealth.mockResolvedValue({
        initialized: false,
        sealed: true,
      });

      await expect(sdk.getSecret('myapp/test')).rejects.toThrow(
  ...
  describe('Zero-Trust Validation', () => {
    it('should ensure no secrets in environment variables', () => {
      ...
      const suspiciousKeys = envVars.filter(
        key =>
      ...
      const allowedTestVars = suspiciousKeys.filter(
        key => key.includes('TEST_') || key.includes('VITEST_')
      );
