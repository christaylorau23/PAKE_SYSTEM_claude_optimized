...
from ..repositories.abstract_repositories import AbstractUserRepository, AbstractSearchHistoryRepository
...
class PAKESystemService:
    ...
    def __init__(
        ...
        self.user_service = UserService(
            self.repository_container.get_user_repository()
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_healthy = (
                repo_health["status"] == "healthy" and
                user_service_health["status"] == "healthy"
            )
...
async def create_service_with_database_session(session: AsyncSession) -> PAKESystemService:
    """Create PAKE System service with database session"""
    return PAKESystemService(session=session)


async def create_service_with_repository_container(container: RepositoryContainer) -> PAKESystemService:
