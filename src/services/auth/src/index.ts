...
class AuthenticationService {
  ...
  private async initializeServices(): Promise<void> {
    ...
    try {
      ...
      const passwordService = new PasswordService(this.redis);
  ...
  private setupBackgroundTasks(): void {
    ...
    // Clean up expired password histories every hour
    setInterval(
      async () => {
        try {
          const cleanedHistories =
            await this.services.passwordService.cleanupExpiredHistories();
          if (cleanedHistories > 0) {
            this.logger.info('Cleaned up expired password histories', {
