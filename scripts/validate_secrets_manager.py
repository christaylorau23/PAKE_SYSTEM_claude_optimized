...
def validate_secrets_manager_usage():
    ...
    IMPROPER_PATTERNS = [
        ...
        (r'os\.getenv\s*\(\s*["\'][^"\']+["\']\s*,\s*["\'][^"\']+["\']\s*\)', "os.getenv with hardcoded fallback"),
        
        # os.environ.get with hardcoded fallbacks
        (r'os\.environ\.get\s*\(\s*["\'][^"\']+["\']\s*,\s*["\'][^"\']+["\']\s*\)', "os.environ.get with hardcoded fallback"),
        
        # Direct environment variable access with fallbacks
        (r'os\.environ\[["\'][^"\']+["\']\]\s*or\s*["\'][^"\']+["\']', "Direct environment access with fallback"),
    ]
                ...
                for line_num, line in enumerate(lines, 1):
                    for pattern, description in IMPROPER_PATTERNS:
                        if re.search(pattern, line):
                            ...
                            if any(word in line.lower() for word in ['test', 'example', 'placeholder', 'mock', 'fake']):
                                continue
    ...
    if violations:
        ...
        print("Replace hardcoded fallbacks with enterprise secrets manager:", file=sys.stderr)
        print("", file=sys.stderr)
        print("❌ BAD:", file=sys.stderr)
        print("   api_key = os.getenv('API_KEY', 'default-api-key')", file=sys.stderr)
        print("", file=sys.stderr)
        print("✅ GOOD:", file=sys.stderr)
        print("   from src.services.secrets_manager.enterprise_secrets_manager import get_api_key", file=sys.stderr)
        ...
        print("       raise ValueError('API_KEY environment variable is required')", file=sys.stderr)
        print("", file=sys.stderr)
