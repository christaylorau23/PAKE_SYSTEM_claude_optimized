...
**Generate 16-character passwords for each service:**
```powershell
...
# Edit with new passwords (use your preferred editor)
notepad .env
# OR
nano .env
```
...
# Start only PostgreSQL to reset password
...
```bash
# Redis password will be updated when services restart with new config
...
```bash
# Search for old passwords in source code
cd D:\Projects\PAKE_SYSTEM
...
- [ ] **No old passwords** found in git history
...
aws secretsmanager create-secret --name "pake-system/database" \
  --secret-string '{"password":"YOUR_NEW_DB_PASSWORD_HERE"}'

aws secretsmanager create-secret --name "pake-system/redis" \
  --secret-string '{"password":"YOUR_NEW_REDIS_PASSWORD_HERE"}'
```

**Option 2: HashiCorp Vault**
```bash
# Store in Vault
vault kv put secret/pake-system/database password="YOUR_NEW_DB_PASSWORD_HERE"
vault kv put secret/pake-system/redis password="YOUR_NEW_REDIS_PASSWORD_HERE"
```

**Option 3: Azure Key Vault**
```bash
# Store in Azure Key Vault  
az keyvault secret set --vault-name "pake-keyvault" --name "db-password" --value "YOUR_NEW_DB_PASSWORD_HERE"
az keyvault secret set --vault-name "pake-keyvault" --name "redis-password" --value "YOUR_NEW_REDIS_PASSWORD_HERE"
```
...
- [ ] **Document all rotated credentials** in secure password manager
...
**Exposed Secrets:**
- [x] PostgreSQL database password (`process.env.SERVICE_PASSWORD || 'SECURE_PASSWORD_REQUIRED'`)
- [x] Redis password (`process.env.REDIS_PASSWORD || 'SECURE_PASSWORD_REQUIRED'`)  
- [x] n8n admin password (`n8n_secure_2024`)
...
- [ ] All passwords rotated and tested
