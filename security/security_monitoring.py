...
class SecurityEvent(BaseModel):
    ...
    false_positive: bool = Field(
        default=False, description="Whether this is a false positive"
    )
...
class SecurityIncident(BaseModel):
    ...
    status: IncidentStatus = Field(
        default=IncidentStatus.OPEN, description="Incident status"
    )
    ...
    events: list[SecurityEvent] = Field(
        default_factory=list, description="Related events"
    )
...
class SecurityMonitoringSystem:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        self.logger.info("Initialized %s default security rules", len(default_rules))
    ...
    async def process_security_event(
        ...
        self.logger.info(
            "Processed security event: %s - %s", event.event_id, event_type
        )
    ...
    async def _matches_rule_pattern(
        self, event: SecurityEvent, rule: SecurityRule
    ) -> bool:
        """Check if event matches rule pattern."""
    ...
    async def _detect_unauthorized_access(self, event: SecurityEvent) -> bool:
        ...
        if event.event_type == "file_access":
            ...
            if (
                "unauthorized" in event.description.lower()
                or "denied" in event.description.lower()
            ):
            ...
            if incident.status in [IncidentStatus.OPEN, IncidentStatus.INVESTIGATING]:
                ...
                for incident_event in incident.events:
                    if (
                        (
                            event.source_ip
                            and incident_event.source_ip == event.source_ip
                        )
                        or (event.user_id and incident_event.user_id == event.user_id)
                        or (
    ...
    async def _create_incident(self, event: SecurityEvent) -> SecurityIncident:
        ...
        self.logger.warning("Created security incident: %s", incident.incident_id)
    ...
    async def update_incident_status(
        ...
        self.logger.info("Updated incident %s status to %s", incident_id, status.value)
        return True

    async def _get_incident_by_id(self, incident_id: str) -> SecurityIncident | None:
        """Get incident by ID."""
        for incident in self.incidents:
    ...
    def _get_top_event_types(self, incidents: list[SecurityIncident]) -> dict[str, int]:
        ...
        return dict(
            sorted(event_type_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        )
    ...
    async def generate_security_report(self, days: int = 30) -> dict[str, Any]:
        ...
        high_events = len(
            [e for e in recent_events if e.threat_level == ThreatLevel.HIGH]
        )
        ...
        return {
            ...
            "recommendations": await self._generate_security_recommendations(
                recent_events, recent_incidents
            ),
        }

    async def _generate_security_recommendations(
        self, events: list[SecurityEvent], incidents: list[SecurityIncident]
    ) -> list[str]:
        ...
        if len(critical_events) > 5:
            recommendations.append(
                "High number of critical security events detected - review security controls"
            )
        ...
        if len(open_incidents) > 3:
            recommendations.append(
                "Multiple unresolved security incidents - prioritize incident response"
            )
