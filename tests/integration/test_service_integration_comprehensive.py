...
from src.services.ingestion.orchestrator import IngestionOrchestrator, IngestionConfig
from src.services.caching.redis_cache_service import RedisCacheService, CacheConfig
from src.services.analytics.advanced_analytics_engine import AdvancedAnalyticsEngine

from tests.factories import (
    UserFactory, UserInDBFactory, SearchQueryFactory, 
    SearchResultFactory, TenantFactory, SessionFactory
)
...
class TestServiceIntegrationComprehensive:
    ...
    async def test_multi_tenant_user_isolation(self, user_service, redis_service):
        ...
        wrong_tenant_user = await user_service.getUserById(user1.id, tenant_id="tenant_2")
    ...
    async def test_ingestion_with_caching_integration(self, ingestion_orchestrator, redis_service):
    ...
    @pytest.mark.integration
    @pytest.mark.integration_cache
    async def test_ingestion_cache_invalidation(self, ingestion_orchestrator, redis_service):
    ...
    async def test_ingestion_with_database_persistence(self, ingestion_orchestrator, test_database):
    ...
    @pytest.mark.integration
    @pytest.mark.integration_cache
    async def test_analytics_with_caching_integration(self, analytics_engine, redis_service):
    ...
    @pytest.mark.integration
    @pytest.mark.integration_database
    async def test_analytics_with_database_integration(self, analytics_engine, test_database):
        """Test analytics service with database integration"""
        # Arrange
        # Simulate some data in the database
        test_data = [
            {"timestamp": datetime.now(timezone.utc), "metric": "user_count", "value": 100},
            {"timestamp": datetime.now(timezone.utc) - timedelta(hours=1), "metric": "user_count", "value": 95},
            {"timestamp": datetime.now(timezone.utc) - timedelta(hours=2), "metric": "user_count", "value": 90}
        ]
    ...
    @pytest.mark.integration_auth
    @pytest.mark.integration_cache
    async def test_user_ingestion_workflow_integration(self, user_service, ingestion_orchestrator, redis_service):
        ...
        session_valid = await user_service.sessionService.validateSession(auth_result.session_id)
    ...
    @pytest.mark.integration_auth
    @pytest.mark.integration_cache
    async def test_analytics_user_behavior_integration(self, user_service, analytics_engine, redis_service):
        ...
        await user_service.updateUserProfile(user.id, {"last_search": "machine learning"})
    ...
    @pytest.mark.integration_database
    @pytest.mark.integration_cache
    async def test_multi_tenant_data_isolation_integration(self, user_service, ingestion_orchestrator, redis_service):
    ...
    async def test_concurrent_service_operations(self, user_service, ingestion_orchestrator, redis_service):
    ...
    async def test_cache_performance_under_load(self, redis_service):
        ...
        # Arrange
        test_data = [{"key": f"perf_test_{i}", "value": f"value_{i}"} for i in range(100)]
        
        # Act: Set operations
        start_time = time.time()
        set_tasks = [redis_service.set(item["key"], item["value"]) for item in test_data]
