...
class SecurityTester:
    ...
    def test_insecure_hash_algorithms(self) -> None:
        ...
        md5_patterns = ["hashlib.md5", "md5(", "MD5", "md5sum"]

        # Search for SHA1 usage
        sha1_patterns = ["hashlib.sha1", "sha1(", "SHA1", "sha1sum"]
    ...
    def test_insecure_serialization(self) -> None:
        ...
        pickle_files = [
            ...
            if "migrate_from_pickle" not in f and "test_" not in f
        ]
    ...
    def test_network_security(self) -> None:
        ...
        insecure_bindings = [
            f
            for f in insecure_bindings
            if not any(
                word in f.lower()
                for word in ["comment", "todo", "fixme", "note", "example"]
            )
        ]
    ...
    def test_hardcoded_secrets(self) -> None:
        ...
        secrets_found = [
            ...
            if not any(
                word in s.lower()
                for word in ["test_", "example", "placeholder", "change-in-production"]
            )
        ]
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        severity_counts = {
            "critical": sum(
                ...
                if not r.passed and r.severity == SecuritySeverity.CRITICAL
            ),
            "high": sum(
                1
                for r in self.results
                if not r.passed and r.severity == SecuritySeverity.HIGH
            ),
            "medium": sum(
                1
                for r in self.results
                if not r.passed and r.severity == SecuritySeverity.MEDIUM
            ),
            "low": sum(
                1
                for r in self.results
                if not r.passed and r.severity == SecuritySeverity.LOW
            ),
        }
