...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _clean_old_requests(self, client_ip: str, current_time: float):
        ...
        while self.burst_requests[client_ip] and self.burst_requests[client_ip][0] < ten_seconds_ago:
        ...
        if client_ip in self.locked_ips:
            if current_time < self.locked_ips[client_ip]:
                raise HTTPException(
                    ...
                    headers={"Retry-After": str(int(self.locked_ips[client_ip] - current_time))}
                )
            else:
