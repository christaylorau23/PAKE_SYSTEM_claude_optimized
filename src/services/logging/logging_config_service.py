...
class LoggingServiceConfig:
    ...
    json_format: bool = field(default_factory=lambda: os.getenv("LOG_JSON", "true").lower() == "true")
    console_enabled: bool = field(default_factory=lambda: os.getenv("LOG_CONSOLE", "true").lower() == "true")
    file_enabled: bool = field(default_factory=lambda: os.getenv("LOG_FILE", "true").lower() == "true")
    ...
    # Security settings
    mask_sensitive_data: bool = field(default_factory=lambda: os.getenv("LOG_MASK_SENSITIVE", "true").lower() == "true")
    security_level: str = field(default_factory=lambda: os.getenv("LOG_SECURITY_LEVEL", "internal"))
    
    # Performance settings
    async_logging: bool = field(default_factory=lambda: os.getenv("LOG_ASYNC", "true").lower() == "true")
    buffer_size: int = field(default_factory=lambda: int(os.getenv("LOG_BUFFER_SIZE", "1000")))
    flush_interval_seconds: int = field(default_factory=lambda: int(os.getenv("LOG_FLUSH_INTERVAL", "5")))
    
    # External services
    syslog_enabled: bool = field(default_factory=lambda: os.getenv("LOG_SYSLOG", "false").lower() == "true")
    syslog_host: str = field(default_factory=lambda: os.getenv("SYSLOG_HOST", "localhost"))
    syslog_port: int = field(default_factory=lambda: int(os.getenv("SYSLOG_PORT", "514")))
    
    # Monitoring integration
    metrics_enabled: bool = field(default_factory=lambda: os.getenv("LOG_METRICS", "true").lower() == "true")
    tracing_enabled: bool = field(default_factory=lambda: os.getenv("LOG_TRACING", "true").lower() == "true")
...
class MonitoringServiceConfig:
    ...
    # Collection settings
    collection_interval_seconds: int = field(default_factory=lambda: int(os.getenv("MONITORING_COLLECTION_INTERVAL", "30")))
    metrics_retention_days: int = field(default_factory=lambda: int(os.getenv("MONITORING_RETENTION_DAYS", "30")))
    health_check_interval_seconds: int = field(default_factory=lambda: int(os.getenv("MONITORING_HEALTH_CHECK_INTERVAL", "60")))
    
    # Thresholds
    cpu_threshold_percent: float = field(default_factory=lambda: float(os.getenv("MONITORING_CPU_THRESHOLD", "80.0")))
    memory_threshold_percent: float = field(default_factory=lambda: float(os.getenv("MONITORING_MEMORY_THRESHOLD", "85.0")))
    disk_threshold_percent: float = field(default_factory=lambda: float(os.getenv("MONITORING_DISK_THRESHOLD", "90.0")))
    response_time_threshold_ms: float = field(default_factory=lambda: float(os.getenv("MONITORING_RESPONSE_TIME_THRESHOLD", "5000.0")))
    
    # Alerting
    alerting_enabled: bool = field(default_factory=lambda: os.getenv("MONITORING_ALERTING", "true").lower() == "true")
    alert_cooldown_minutes: int = field(default_factory=lambda: int(os.getenv("MONITORING_ALERT_COOLDOWN", "15")))
    max_alerts_per_hour: int = field(default_factory=lambda: int(os.getenv("MONITORING_MAX_ALERTS_PER_HOUR", "10")))
    
    # External services
    prometheus_enabled: bool = field(default_factory=lambda: os.getenv("MONITORING_PROMETHEUS", "false").lower() == "true")
    prometheus_port: int = field(default_factory=lambda: int(os.getenv("PROMETHEUS_PORT", "9090")))
    
    datadog_enabled: bool = field(default_factory=lambda: os.getenv("MONITORING_DATADOG", "false").lower() == "true")
    datadog_api_key: str = field(default_factory=lambda: os.getenv("DATADOG_API_KEY", ""))
    datadog_app_key: str = field(default_factory=lambda: os.getenv("DATADOG_APP_KEY", ""))
    
    grafana_enabled: bool = field(default_factory=lambda: os.getenv("MONITORING_GRAFANA", "false").lower() == "true")
...
class LoggingConfigService:
    ...
    def __init__(self, config_file: str = None):
        ...
        self.config_sources: List[ConfigSource] = [ConfigSource.ENVIRONMENT, ConfigSource.FILE]
    ...
    def validate_environment(self) -> Dict[str, Any]:
        ...
        validation_results = {
            'timestamp': datetime.now(UTC).isoformat(),
            'environment': self.logging_config.environment if self.logging_config else 'unknown',
            'validations': [],
            'warnings': [],
            'errors': []
        }
        ...
        if self.logging_config and self.logging_config.file_enabled:
            ...
            if not log_dir.exists():
                try:
                    ...
                    validation_results['validations'].append("Log directory created successfully")
                except Exception as e:
                    validation_results['errors'].append(f"Failed to create log directory: {e}")
            else:
                validation_results['validations'].append("Log directory exists and is accessible")
        
        # Check external service configurations
        if self.monitoring_config:
            if self.monitoring_config.datadog_enabled:
                if not self.monitoring_config.datadog_api_key:
                    validation_results['warnings'].append("Datadog enabled but API key not configured")
                else:
                    validation_results['validations'].append("Datadog configuration appears valid")
            
            if self.monitoring_config.prometheus_enabled:
                validation_results['validations'].append("Prometheus monitoring enabled")
        ...
        if self.logging_config:
            if not self.logging_config.mask_sensitive_data:
                validation_results['warnings'].append("Sensitive data masking is disabled")
            
            if self.logging_config.security_level.lower() == 'public':
                validation_results['warnings'].append("Security level is set to public - consider using internal or higher")
