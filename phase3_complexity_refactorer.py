...
class Phase3ComplexityRefactorer:
    ...
    def refactor_function(self, file_path: Path, function_name: str, line_start: int) -> Tuple[bool, int]:
        ...
        try:
            ...
            refactored_content = self._apply_refactoring_patterns(content, target_function, function_name)
    ...
    def _apply_refactoring_patterns(self, content: str, function_node: ast.AST, function_name: str) -> str:
        """Apply systematic refactoring patterns to reduce complexity."""
        lines = content.splitlines()
        start_line = function_node.lineno - 1
        end_line = function_node.end_lineno - 1 if hasattr(function_node, 'end_lineno') else start_line + 50
        ...
        # Apply refactoring patterns
        refactored_function = self._extract_method_pattern(function_content, function_name)
        
        # Replace in original content
        new_lines = lines[:start_line] + refactored_function.splitlines() + lines[end_line + 1:]
        return '\n'.join(new_lines)
    ...
    def _extract_method_pattern(self, function_content: str, function_name: str) -> str:
        ...
        extracted_method = self._create_extracted_method(lines[block_start:block_end], function_name, block_type)
        
        # Replace complex block with method call
        new_lines = lines[:block_start]
        new_lines.append(f"        {self._generate_method_call(function_name, block_type)}")
        ...
        for i, line in enumerate(lines):
            ...
            if (stripped.startswith('if ') or stripped.startswith('elif ') or 
                ...
                if nested_count > 3:  # Threshold for complexity
                    complex_blocks.append((block_start, i + nested_count, 'conditional'))
                    block_start = None
    ...
    def _create_extracted_method(self, block_lines: List[str], parent_function: str, block_type: str) -> str:
        ...
        method_content = f"""    def {method_name}(self) -> None:
    ...
    def refactor_priority_functions(self) -> Dict[str, Any]:
        ...
        priority_functions = [f for f in self.target_functions if f['priority'] in ['P0', 'P1']]
        ...
        for func_info in priority_functions:
            ...
            success, fixes = self.refactor_function(file_path, function_name, line_start)
        ...
        return {
            ...
            'success_rate': (refactored_count / len(priority_functions) * 100) if priority_functions else 0
        }
