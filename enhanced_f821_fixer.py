        ...
        for param_name, line_numbers in analysis.items():
            ...
            for line_num in line_numbers:
                if line_num <= len(lines):
                    ...
                    if f'def ' in line and f'({param_name}' in content:
                        ...
                        for i in range(line_idx, max(0, line_idx - 20), -1):
                            func_line = lines[i]
                            if func_line.strip().startswith('def ') and '(' in func_line and ')' in func_line:
                                # Found function definition, add parameter
                                if param_name not in func_line:
                                    # Add parameter to function signature
                                    if func_line.endswith(') ->'):
                                        new_line = func_line.replace(') ->', f', {param_name}: {param_type}) ->')
                                    elif func_line.endswith('):'):
                                        new_line = func_line.replace('):', f', {param_name}: {param_type}):')
                                    elif func_line.endswith(') -> None:'):
                                        new_line = func_line.replace(') -> None:', f', {param_name}: {param_type}) -> None:')
                                    else:
                                        new_line = func_line.replace(')', f', {param_name}: {param_type})')
