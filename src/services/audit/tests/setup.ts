...
expect.extend({
  toBeValidAuditEvent(received: any) {
    ...
    if (missingFields.length > 0) {
      return {
        message: () =>
          `Expected audit event to have required fields. Missing: ${missingFields.join(', ')}`,
        pass: false,
      };
    }
    ...
    if (
      ...
      !received.action.resource ||
      !received.action.result
    ) {
      return {
        message: () =>
          'Expected audit event action to have type, resource, and result',
        pass: false,
      };
    }
    ...
    if (!received.context.environment || !received.context.application) {
      return {
        message: () =>
          'Expected audit event context to have environment and application',
        pass: false,
      };
    }
  ...
  toBeWithinTimeRange(received: any, start: Date, end: Date) {
    ...
    if (timestamp < start || timestamp > end) {
      return {
        message: () =>
          `Expected timestamp ${timestamp.toISOString()} to be within range ${start.toISOString()} - ${end.toISOString()}`,
        pass: false,
      };
    }
...
export class TestHelpers {
   ...
   */
  static createMockAWSResponse(data: any = {}): {
    promise: () => Promise<any>;
  } {
    return {
